/**
 * FusionCharts: Flash Player detection and Chart embedding. Version: 1.2.4
 * (16th February, 2009) - Added fix for chart with % width/height. Version:
 * 1.2.3 (15th September, 2008) - Added fix for % and & characters. Additional
 * fixes to properly handle double quotes and single quotes in setDataXML()
		...
		if ((this.detectFlashVersion == 1) && (this.installedVer.major < 8)) {
			if (this.autoInstallRedirect == 1) {
				...
				var installationConfirm = window
						.confirm("You need Adobe Flash Player 8 (or above) to view the charts. It is a free and lightweight installation from Adobe.com. Please click on Ok to install the same.");
			...
			// If loaded in IE and scaleMode and width/height specified in %,
			// load the chart using onload event
			if (this.getVariable('scaleMode').search(/noscale/i) >= 0
					&& (this.getAttribute('width').search("%") > 0 || this
							.getAttribute('height').search("%") > 0)) {
...
infosoftglobal.FusionChartsUtil.getPlayerVersion = function() {
	...
	if (navigator.plugins && navigator.mimeTypes.length) {
		...
		if (axo != null) {
			PlayerVersion = new infosoftglobal.PlayerVersion(axo
